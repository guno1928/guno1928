...
**ALOSWALL**
The in-kernel DDoS firewall behind ALOS. It runs in XDP, the kernel's earliest packet path, and rules on every single packet in around 150 nanoseconds. Fully stateful, SYN cookies in the fast path, live rule changes with zero downtime, and a custom BPF map built to stay fast under attack load instead of collapsing under it.
